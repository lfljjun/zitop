...
public class CkFileUploadServlet extends HttpServlet
{
	/**
	 * 
	 */
	private static final long serialVersionUID = 1768197595637052327L;
	private final static Log log = LogFactory.getLog(CkFileUploadServlet.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		try
		{
			...
			int maxSize = ParamaterValues.getInteger(ConfigNameConstants.FILE_MAX_SIZE);
			// 设置最大上传的文件大小 3M
			...
			String fileUrl = ParamaterValues.getString(ConfigNameConstants.FILE_DOWNLOAD_URL) + dsr + "/" + fileName;
		...
		catch (Exception e)
		{
			...
			{
				responseError(out,"上传文件超出范围，最大不能超过:"+ParamaterValues.getInteger(ConfigNameConstants.FILE_MAX_SIZE)+" M");
			}
			else
			{
